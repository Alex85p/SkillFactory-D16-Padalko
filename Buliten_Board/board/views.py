...
class ListingsAll(ListView):
    """Класс представления для отображения списка всех объявлений."""
    model = Listing
    ...
    def get_context_data(self, **kwargs):
        """Возвращает словарь с контекстом данных, содержащий список объявлений и информацию о наличии у текущего
        пользователя откликов на каждое объявление. Кроме того, в контекст добавляется текущее время сервера."""
...
class ListingDetail(DetailView):
    """Представление для просмотра объявления"""
    model = Listing
    ...
    def resize_image(image_field, new_width, new_height):
        """метод для изменения размеров изображений под формат портала"""
    ...
    def get_context_data(self, **kwargs):
        """Возвращает словарь с контекстом данных, содержащий объект объявления и информацию о наличии у текущего
        пользователя откликов на это объявление. Если в объявлении есть изображения, то меняет их размер под формат
        портала """
        context = super().get_context_data(**kwargs)
...
class AuthorListings(LoginRequiredMixin, ListingsAll):
    """Класс представления для отображения списка всех объявлений автора в личном кабинете."""
    model = Listing
...
class ReplyDetail(DetailView):
    """Представление для просмотра отклика на объявление"""
    model = Reply
...
class ListingReply(ListView):
    """Представление для просмотра всех откликов на объявление в личном кабинете"""
    model = Reply
...
class AuthorReplies(LoginRequiredMixin, ListView):
    """Представление для просмотра  в личном кабинете всех откликов, которые оставил пользователь"""
    model = Reply
...
def accept_reply(request, pk):
    """Представление для принятия отклика на объявление."""
...
def reject_reply(request, pk):
    """Представление для отклонения и удаления отклика на объявление."""
    reply = get_object_or_404(Reply, id=pk)
    reply.delete()
...
def cancel_reply(request, pk):
    """
    Представление для отмены решения по отклику на объявление.
    Реализовал отправку уведомления здесь, потому что при создании нового отклика его status = False
    и отправляется ложное уведомление об отмене отклика, так как сигнал реагирует на post_save
    """
    reply = get_object_or_404(Reply, id=pk)
    reply.status = False
    reply.save()
...
def reply_action(request, pk):
    """
    Представление для работы с откликами в личном кабинете.
    Реализованы функции прошлых трех, просто другим методом и все три в одном представлении
    Сделано просто для тренировки
    """
